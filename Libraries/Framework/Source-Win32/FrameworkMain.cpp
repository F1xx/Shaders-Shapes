    ...
    while( !done )
    {
        ...
        else
        {
            ...
            if( m_WindowIsActive )
            {
                ...
                {
                    //Send the event manager to go tell shit to work
...
vec2 Framework::GetWindowCenter()
{
	return vec2((m_CurrentWidth * 0.5f), (m_CurrentHeight * 0.5f));
}
    ...
    case WM_KEYDOWN:
        {
            ...
            if( keyWasPressedLastTimeMessageArrived == false )
            {
                ...
                //CREATE A KEYDOWN EVENT HERE
                
                int x, y;
                pFramework->GetMouseCoordinates(&x, &y);
                InputEvent* event = new InputEvent(DeviceType_Keyboard, wParam, KeyState_Down, ivec2(x, y));
                EventManager* temp = pFramework->GetEventManager();
                pFramework->GetEventManager()->AddEvent(event);
            }
        }
        return 0;
    ...
    case WM_KEYUP:
        {
            ...
            int x, y;
            pFramework->GetMouseCoordinates(&x, &y);
            InputEvent* event = new InputEvent(DeviceType_Keyboard, wParam, KeyState_Up, ivec2(x, y));
            EventManager* temp = pFramework->GetEventManager();
            pFramework->GetEventManager()->AddEvent(event);
        }
        return 0;
